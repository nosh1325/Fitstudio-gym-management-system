...
// Initialize variable to determine whether to show the form or not
$showForm = true;

// Fetching plan ID and member name from user input
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Check if the member username exists in the purchases table
    ...
    if ($username_count > 0) {
        // Check if the member has purchased the specified plan
        $check_purchase_sql = "SELECT COUNT(*) AS count FROM Purchases WHERE wplan_id = $plan_id AND customer_username = '$member_name'";
        $purchase_result = $conn->query($check_purchase_sql);

        $purchase_row = $purchase_result->fetch_assoc();
        $purchase_count = $purchase_row["count"];

        if ($purchase_count > 0) {
            // Fetching data based on plan ID
            if ($plan_id == 1 || $plan_id == 2) {
                // Fetching article data from the database based on plan ID
                $article_sql = "SELECT article_links FROM Workout_plan_2 WHERE plan_id = $plan_id";
                ...
                if ($article_result->num_rows > 0) {
                    ...
                    while ($article_row = $article_result->fetch_assoc()) {
                        $article_link = $article_row["article_links"];
                        // Check if article link is not NULL
                ...
                // Fetching diet chart data from the database based on plan ID
                ...
                // Hide the form as it's no longer needed
                $showForm = false;
            } else {
...
function getArticlePreview($url)
{
    ...
    foreach ($paragraphs as $paragraph) {
        ...
        if ($word_count + $words > 200) { // Adjust the word count limit here (e.g., 200 words)
            // If adding the words from this paragraph exceeds the limit, break
            break;
        }
